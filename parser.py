...
interned_string = sys.intern('some_string')
...
# from your_code import read_pos, read_conll, Parser, train
...
def main_train(model_dir, train_loc):
    ...
    parser = Parser(load=False)
    train(parser, sentences, nr_iter=15)
    parser.save(os.path.join(model_dir, 'parser.model'))
    print(f'Model saved to {model_dir}/parser.model')

def main_test(model_dir, heldout_in, heldout_gold):
    input_sents = list(read_pos(heldout_in))
    parser = Parser(load=True, model_path=os.path.join(model_dir, 'parser.model'))
    gold_sents = list(read_conll(heldout_gold))
    c, t = 0, 0
    t1 = time.time()
    for (words, tags), (_, _, gold_heads, gold_labels) in zip(input_sents, gold_sents):
...
def main_parse(model_dir):
    ...
    for sentence in sentences:
        _, heads = parser.parse(sentence)
        print(heads)

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("python3 parser.py train MODEL DIR TRAIN DATA")
        print("python3 parser.py test MODEL DIR VAL DATA")
        print("python3 parser.py parse MODEL DIR < tokenized input sentences.txt")
        print("Usage:")
    ...
    if command == "train":
        ...
        print("python3 parser.py train MODEL DIR TRAIN DATA")
        sys.exit(1)
        train_loc = sys.argv[3]
        main_train(model_dir, train_loc)

    elif command == "test":
        if len(sys.argv) != 5:
        print("python3 parser.py test MODEL DIR VAL DATA")
        sys.exit(1)
        heldout_in = sys.argv[3]
        heldout_gold = sys.argv[4]
        main_test(model_dir, heldout_in, heldout_gold)
