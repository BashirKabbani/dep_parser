...
class Parser(object):
    ...
    def train_one(self, itn, words, gold_tags, gold_heads):
        print(itn, words, gold_tags, gold_heads)
        print('==============================')
        n = len(words)
        i = 2; stack = [1]; parse = Parse(n)
        tags = self.tagger.tag(words)
        while stack or (i + 1) < n:
            ...
            valid_moves = get_valid_moves(i, n, len(stack))
            print(words)
            print(i, n, stack, parse.heads, gold_heads)
...
class PerceptronTagger(object):
    ...
    def save(self):
        ...
        with open(PerceptronTagger.model_loc, 'wb') as f:
            pickle.dump((self.model.weights, self.tagdict, self.classes), f, -1)
...
def read_conll(loc):
    for sent_str in open(loc).read().strip().split('\n\n'):
        ...
        for i, (word, pos, head, label) in enumerate(lines):
            ...
            #words.append(sys.intern(normalize(word)))
...
def main_train(model_dir, train_loc):
    ...
    parser.save()
    print(f"Training completed and model saved to {model_dir}")


def main_test(model_dir, heldout_in, heldout_gold):
    input_sents = list(read_pos(heldout_in))
    parser = Parser(load=True)  # Load the saved model
    c = 0
    t = 0
...
if __name__ == '__main__':
    ...
    if mode == 'train':
        ...
        print('training ....')
        main_train(model_dir, train_data_path)
    elif mode == 'test':
        val_data_path = sys.argv.sys[3]
        print('testing ....')
        # main_test(model_dir, val_data_path)
    elif mode == 'parse':
        print('parse')
    else:
